...
const POSView: React.FC = () => {
  ...
  const handleStartShift = async () => {
    ...
    try {
      ...
      setShiftTotals({
        ...
        cashIn: 0,
        cashOut: 0,
        expected: parseFloat(startCash),
      });
      setShowStartShiftModal(false);
    } catch (error: any) {
      console.error(error);
      if (error?.message?.includes('violates row-level security')) {
        showErrorToast('Permission Denied: Please sign in with an authorized account.');
      } else {
        showErrorToast('Failed to start shift: ' + (error?.message || 'Unknown error'));
      }
    } finally {
  ...
  const handleCashMovement = async () => {
    ...
    try {
      ...
      console.error(error);
      if (
        error?.message?.includes('relation "cash_movements" does not exist') ||
        error?.code === '42P01'
      ) {
        showErrorToast(
          'System Update Required: Please ask administrator to run the database setup script (enable_cash_features.sql).'
        );
      } else {
  ...
  const confirmCloseShift = async () => {
    ...
    try {
      ...
      if (error?.message?.includes('column') || error?.code === '42703') {
        showErrorToast(
          "System Update Required: Database schema out of date. Run 'enable_cash_features.sql'."
        );
      } else {
  ...
  const toggleAddOn = (addOn: AddOn) => {
    ...
    if (isSelected) {
      setTempCustoms({
        ...tempCustoms,
        selectedAddOns: tempCustoms.selectedAddOns?.filter((ao) => ao.id !== addOn.id),
      });
    } else {
      setTempCustoms({
        ...tempCustoms,
        selectedAddOns: [...(tempCustoms.selectedAddOns || []), addOn],
      });
    }
  };
  ...
  const warnIfLowStock = (item: InventoryItem, nextQty: number) => {
    ...
    if (remaining >= 0 && remaining <= 5) {
      showErrorToast(
        t.lastPiecesWarning.replace('{count}', remaining.toString()).replace('{item}', item.name)
      );
    }
  };
  ...
  const addToCart = (item: InventoryItem, customs?: BeverageCustomization) => {
    ...
    const addOnsExtra =
      customs?.selectedAddOns?.reduce((sum: number, ao: AddOn) => sum + ao.price, 0) || 0;

    const sizeMultiplier = customs
      ? SIZE_MULTIPLIERS[customs.size as keyof typeof SIZE_MULTIPLIERS]
      : 1.0;
    ...
    setCart((prev) => {
      const addOnsIds =
        customs?.selectedAddOns
          ?.map((ao: AddOn) => ao.id)
          .sort()
          .join(',') || '';
      const cartItemId = customs
        ? `${item.id}-${customs.size}-${customs.milkType}-${customs.sugarLevel}-${addOnsIds}`
        : item.id;
    ...
    try {
      ...
      if (selectedCustomer) {
        try {
          ...
          // Non-critical: loyalty update failed, checkout succeeded
        }
      }
  ...
  const handlePrint = async (transaction?: TransactionData) => {
    if (transaction) {
      ...
      if (user?.role !== 'ADMIN' && user?.role !== 'MANAGER') {
        showErrorToast(t.reprintAuthRequired);
        return;
      }
        ...
        await supabase.from('reprint_logs').insert([
          {
            ...
            user_id: isDemoMode() ? null : user?.id,
      ...
      scheduleTimeout(() => window.print(), 200);
  ...
  const searchInvoice = async () => {
    ...
    try {
      ...
      if (error || !data) {
        showErrorToast(t.invoiceNotFound);
        return;
      }

      if (data.is_returned) {
        showErrorToast(t.invoiceAlreadyReturned);
        return;
      }
  ...
  const updateReturnReason = (cartId: string, reason: string) => {
    setItemsToReturn((prev) =>
      prev.map((i) => (i.cartId === cartId ? { ...i, return_reason: reason } : i))
    );
  };
  ...
  const submitReturnRequest = async () => {
    ...
    if (itemsToReturn.some((i) => !i.return_reason)) {
      showErrorToast(t.returnReasonRequired);
      return;
    }
    ...
    try {
      ...
      const returnData = {
        ...
        requested_by_id: isDemoMode() ? null : user?.id,
  ...
  const approveRefund = async (requestId: string, approved: boolean) => {
    if (user?.role !== 'ADMIN' && user?.role !== 'MANAGER') {
      showErrorToast(t.notAuthorizedApproveReturns);
      return;
    }
    ...
    try {
      ...
      const { data: request, error: reqError } = await supabase
        ...
        .update({
          status,
          manager_id: isDemoMode() ? null : user?.id,
          ...
          if (item.type !== 'BEVERAGE') {
            ...
            if (dbItem) {
              ...
              if (sourceItem) {
                ...
                if (createError)
                  console.error(
                    `Failed to create item in location ${returnLocationId}: ${item.name}`,
                    createError
                  );
              } else {
                console.warn(
                  `[WARN] Original item not found, cannot create in new location: ${item.name}`
                );
              }
            }
          }
  ...
  const filteredItems = useMemo(() => {
    return inventoryItems.filter((item) => {
      ...
      const matchesTab =
        activeTab === 'ALL' ||
        (activeTab === 'PACKAGED' && item.category === 'PACKAGED') ||
        (activeTab === 'DRINKS' && item.type === 'BEVERAGE');
  ...
  const splitRemaining =
    totals.total - (splitBreakdown.cash + splitBreakdown.card + splitBreakdown.mobile);
  ...
  return (
    ...
    >
      ...
      <div
        id="thermal-receipt"
        className="hidden bg-white text-black p-4 font-mono leading-normal"
        style={{
          width: settings.printer_width,
          fontSize: settings.printer_width === '58mm' ? '11px' : '13px',
        }}
      >
        <div className="text-center mb-6">
          <div className="flex justify-center mb-3">
            <div
              className={`${settings.printer_width === '58mm' ? 'w-12 h-12' : 'w-16 h-16'} bg-orange-600 rounded-full flex items-center justify-center text-white`}
            >
              <Coffee size={settings.printer_width === '58mm' ? 24 : 32} />
            </div>
          </div>
          <div
            className={`${settings.printer_width === '58mm' ? 'text-lg' : 'text-xl'} font-black uppercase mb-1`}
          >
            {settings.store_name || t.appName}
          </div>
          <div className="text-[10px] opacity-80">
            {t.storeCity} | {settings.store_phone || t.storePhone}
          </div>
          ...
          <>
            <div className="space-y-2 mb-4">
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {new Date(lastReturnRequest.created_at).toLocaleString(
                    lang === 'ar' ? 'ar-QA' : 'en-US'
                  )}
                </span>
              </div>
            ...
            <table className="w-full mb-4 border-collapse">
              <thead>
                <tr className="border-b border-orange-600">
                  <th className="text-left py-2 font-black uppercase text-[10px]">
                    {t.returnedItem}
                  </th>
            ...
            <div className="border-t-2 border-orange-600 pt-4 mb-6">
              <div className="flex justify-between text-lg font-black">
                ...
                <span>
                  {lastReturnRequest.total_refund_amount.toFixed(2)} {t.currency}
                </span>
              </div>
          ...
          <>
                    ...
                    <td className="py-3 pr-2">
                      ...
                      {item.selectedCustomizations && (
                        <div className="text-[9px] italic opacity-70 mt-0.5">
                          ...
                          {item.selectedCustomizations.milkType !== 'Full Fat'
                            ? `, ${item.selectedCustomizations.milkType}`
                            : ''}
                          {item.selectedCustomizations.selectedAddOns?.length > 0
                            ? `, +${item.selectedCustomizations.selectedAddOns.length} ${t.extras}`
                            : ''}
                        </div>
                      )}
                    </td>
            ...
            <div className="space-y-2 text-[11px] mb-4">
              <div className="flex justify-between">
                ...
                <span className="font-bold">
                  {lastTransaction?.subtotal?.toFixed(2) || lastTransaction?.total.toFixed(2)}{' '}
                  {t.currency}
                </span>
              </div>
              {(lastTransaction?.vat_amount > 0 || settings.vat_rate > 0) && (
                <div className="flex justify-between">
                  <span className="opacity-70">
                    {t.tax} ({(settings.vat_rate * 100).toFixed(0)}%)
                  </span>
                  <span className="font-bold">
                    {lastTransaction?.vat_amount?.toFixed(2) ||
                      (lastTransaction?.total * settings.vat_rate).toFixed(2)}{' '}
                    {t.currency}
                  </span>
                </div>
              )}
              {lastTransaction?.discount_amount > 0 && (
                <div className="flex justify-between text-green-600">
                  ...
                  <span className="font-bold">
                    -{lastTransaction.discount_amount.toFixed(2)} {t.currency}
                  </span>
                </div>
              )}
              <div className="flex justify-between text-lg font-black pt-2 border-t-2 border-orange-600 mt-2">
                <span>{t.total}</span>
                <span>
                  {lastTransaction?.total.toFixed(2)} {t.currency}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2 text-[10px] mb-6">
              ...
              <div className="flex justify-between pt-2 border-t border-dotted border-orange-100">
                ...
                <span className="font-bold">
                  {lastTransaction?.received_amount?.toFixed(2) ||
              ...
              {lastTransaction?.change_amount > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-black text-black">
                    {lastTransaction.change_amount.toFixed(2)}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="text-center mt-10">
          ...
          <div className="text-[9px] font-black opacity-40 uppercase tracking-[0.2em] mb-2">
            doharoastery.com
          </div>
          ...
          <div className="bg-white  rounded-[48px] max-w-md w-full p-10 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-white border-2 border-orange-600 text-black rounded-3xl">
                  <CreditCard size={28} />
                </div>
            ...
            <div className="bg-white  p-6 rounded-[32px] mb-8 border border-orange-50 ">
              ...
              <div className="text-4xl font-black text-black ">
                ...
                <span className="text-sm opacity-50 uppercase">{t.currency}</span>
              </div>
            </div>
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase text-black block">
                {t.transactionReferenceOptional}
              </label>
              <input
                type="text"
                autoFocus
              ...
              {parseFloat(cashReceived) >= totals.total && (
                <div className="bg-white border-2 border-orange-600 p-4 rounded-2xl flex justify-between items-center animate-in fade-in slide-in-from-top-2">
                  ...
                  <span className="text-2xl font-black text-orange-600 font-mono">
                    {(parseFloat(cashReceived) - totals.total).toFixed(2)} {t.currency}
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-2">
                {[10, 20, 50, 100, 200, 500].map((val) => (
                  <button
                    ...
                    className="py-4 bg-white  rounded-2xl font-black text-sm  transition-colors"
                  >
                    {val}
                  </button>
                ))}
      ...
      {showSplitModal && (
        <div className="fixed inset-0 bg-white/80 backdrop-blur-xl z-[250] flex items-center justify-center p-4">
          <div className="bg-white  rounded-[48px] max-w-lg w-full p-10 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-white  text-black  rounded-3xl">
                  <Scissors size={28} />
                </div>
            ...
            <div className="bg-white  p-6 rounded-[32px] mb-8 border border-orange-50 ">
              ...
              <div className="flex justify-between items-end">
                ...
                <span
                  className={`text-3xl font-black font-mono ${splitRemaining > 0 ? 'text-black' : 'text-green-600'}`}
                >
            ...
            <div className="space-y-6">
              {(
                [
                  { id: 'cash', icon: Banknote, color: 'black' },
                  { id: 'card', icon: CreditCard, color: 'black' },
                  { id: 'mobile', icon: Smartphone, color: 'black' },
                ] as any
              ).map((method: any) => (
                <div key={method.id} className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-black flex items-center gap-2">
                    <method.icon size={14} className="text-orange-600" />{' '}
                    {t[method.id as keyof typeof t] || method.id}
                  </label>
                  <input
                    type="number"
                    value={(splitBreakdown as any)[method.id] || ''}
                    onChange={(e) =>
                      setSplitBreakdown({
                        ...splitBreakdown,
                    ...
                    className="w-full bg-white  border-none rounded-2xl px-6 py-4 font-mono font-bold text-lg outline-none focus:ring-2 focus:ring-stone-500"
                    placeholder="0.00"
                  />
                  ...
                  {method.id === 'card' && splitBreakdown.card > 0 && (
                    <div className="mt-2 animate-in fade-in slide-in-from-top-2">
                      <input
                        ...
                        onChange={(e) =>
                          setSplitBreakdown({ ...splitBreakdown, card_reference: e.target.value })
                        }
        ...
        <div className="fixed inset-0 bg-white/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white  rounded-[40px] max-w-2xl w-full p-8 shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh] custom-scrollbar">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-white border-2 border-orange-600 text-black rounded-2xl">
                  <Coffee size={32} />
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(['S', 'M', 'L'] as const).map((s) => (
                    <button
                      key={s}
                      onClick={() => setTempCustoms({ ...tempCustoms, size: s })}
                      className={`py-5 rounded-3xl font-black transition-all border-2 ${tempCustoms.size === s ? 'bg-orange-600 text-white border-orange-600 shadow-lg' : 'bg-white  border-transparent text-black'}`}
                    >
                      {s}
                    </button>
                  ))}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {Object.keys(MILK_PRICES).map((m) => (
                    <button
                      key={m}
                      onClick={() => setTempCustoms({ ...tempCustoms, milkType: m as any })}
                      className={`px-2 py-4 rounded-2xl font-bold text-[10px] transition-all border-2 ${tempCustoms.milkType === m ? 'bg-orange-600 text-white border-orange-600' : 'bg-white  border-transparent text-black'}`}
                    >
                      {m}
                      {MILK_PRICES[m as keyof typeof MILK_PRICES] > 0 && (
                        <span className="block mt-1 opacity-60">
                          +{MILK_PRICES[m as keyof typeof MILK_PRICES]} {t.currency}
                        </span>
                      )}
                    </button>
                  ))}
...
<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {(customizingItem as any).add_ons.map((ao: AddOn) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex justify-between items-center px-4 py-4 rounded-3xl border-2 transition-all ${isSelected ? 'bg-white border-orange-600 text-black shadow-sm' : 'bg-white  border-transparent text-black'}`}
                        >
                          <span className="text-xs font-bold">{ao.name}</span>
                          <span className="font-mono font-black text-[10px]">
                            +{ao.price} {isSelected && <Check size={12} className="inline ml-1" />}
                          </span>
                        </button>
                      );
                    })}
              ...
              <button
                onClick={() => {
                  ...
                  const milkExtra =
                    MILK_PRICES[tempCustoms.milkType as keyof typeof MILK_PRICES] || 0;
                  const addOnsExtra =
                    tempCustoms.selectedAddOns?.reduce(
                      (sum: number, ao: AddOn) => sum + ao.price,
                      0
                    ) || 0;
                  addToCart(customizingItem, {
                    ...tempCustoms,
                    extraPrice: milkExtra + addOnsExtra,
                  });
                }}
            ...
            {activeTab === 'HISTORY' && (
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                <div className="flex flex-col sm:flex-row items-center gap-2 bg-white px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-orange-50 w-full sm:w-auto">
                  ...
                  {(dateRange.start || dateRange.end) && (
                    <button
                      ...
                      className="text-black hover mt-1 sm:mt-0"
                    >
                      <X size={14} />
                    </button>
                  )}
                ...
                <button
                  ...
                  className="p-3 bg-orange-600 text-white rounded-xl  transition-all border-2 border-orange-600 w-full sm:w-auto"
                >
                  <RefreshCw size={20} />
                </button>
              </div>
            ...
            <div className="space-y-3 animate-in slide-in-from-bottom-4">
              {isLoading ? (
                ...
                filteredHistory.map((tx) => (
                  <div
                    key={tx.id}
                    className="bg-white  p-4 rounded-2xl border border-orange-100  shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row justify-between items-center gap-4 group"
                  >
                    <div className="flex items-center gap-4 w-full md:w-auto">
                      <div className="p-3 bg-white  text-black rounded-xl group- group- transition-colors">
                        <Receipt size={20} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-sm">{tx.id}</h4>
                          <span className="px-2 py-0.5 bg-white  rounded text-[10px] font-black uppercase">
                            {tx.payment_method}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-black text-[10px] mt-1 font-bold">
                          ...
                          <span className="flex items-center gap-1">
                            <UserIcon size={10} /> {tx.cashier_name}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                      <div className="text-right">
                        <span className="block text-[10px] font-black text-black uppercase">
                          {t.total}
                        </span>
                        <span className="text-lg font-black text-black  font-mono">
                          {tx.total.toFixed(2)}{' '}
                          <span className="text-[10px] opacity-50">{t.currency}</span>
                        </span>
                      </div>
                      <button
            ...
            <div className="space-y-8 animate-in slide-in-from-bottom-4 pb-20">
              ...
              <div className="bg-white  p-8 rounded-[40px] border border-orange-100  shadow-sm">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-black font-bold uppercase tracking-widest">
                      {t.searchInvoicePrompt}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <div className="relative flex-1">
                    <Search
                      className={`absolute ${t.dir === 'rtl' ? 'right-6' : 'left-6'} top-1/2 -translate-y-1/2 text-black`}
                      size={20}
                    />
                    <input
                      type="text"
              ...
              {selectedInvoice && (
                <div className="bg-white  rounded-[40px] border border-orange-100  shadow-xl overflow-hidden animate-in zoom-in-95">
                  <div className="bg-orange-600 text-white p-6 flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <span className="block text-[10px] opacity-60 font-black uppercase">
                        {t.total}
                      </span>
                      <span className="text-xl font-black text-black font-mono">
                        {selectedInvoice.total.toFixed(2)} {t.currency}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      {selectedInvoice.items.map((item: any) => {
                        ...
                        return (
                          <div
                            key={item.cartId}
                            className={`p-5 rounded-[28px] border-2 transition-all ${isSelected ? 'border-orange-600 bg-white/30 /10' : 'border-orange-50 '}`}
                          >
                              ...
                              <div className="mt-4 pt-4 border-t border-orange-600/50 /30 animate-in slide-in-from-top-2 space-y-3">
                                <div className="flex items-center justify-between gap-4">
                                  <div className="flex-1">
                                    <label className="text-[10px] font-black text-black uppercase mb-1 block">
                                      {t.quantity}
                                    </label>
                                    <div className="flex items-center gap-2 bg-white  rounded-xl p-1 w-fit">
                                      <button
                                        onClick={() =>
                                          updateReturnQuantity(item.cartId, -1, item.quantity)
                                        }
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <select
                                      ...
                                      onChange={(e) =>
                                        updateReturnReason(item.cartId, e.target.value)
                                      }
                          ...
                          <div>
                            ...
                            <h4 className="text-3xl font-black text-black  font-mono">
                              {itemsToReturn
                                ...
                                .toFixed(2)}
                              <span className="text-xs ml-2 opacity-50 uppercase">
                                {t.currency}
                              </span>
                            </h4>
                          </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-4">
                  ...
                  <button onClick={fetchReturnRequests} className="text-black  transition-colors">
                    <RefreshCw size={18} />
                  </button>
                </div>
                ...
                {isLoading && returnRequests.length === 0 ? (
                  ...
                  returnRequests.map((req) => (
                    <div
                      key={req.id}
                      className="bg-white  p-6 rounded-[32px] border border-orange-100  shadow-sm flex items-center justify-between gap-4"
                    >
                      <div className="flex items-center gap-4">
                        <div
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-[6px] text-[8px] font-black uppercase ${
                                req.status === 'APPROVED'
                                  ? 'bg-white text-black border border-orange-600'
                                  : req.status === 'REJECTED'
                                    ? 'bg-orange-600 text-white'
                          ...
                          <p className="text-[10px] text-black font-bold mt-1">
                            {new Date(req.created_at).toLocaleString()} • {req.items.length}{' '}
                            {t.items}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <span className="block text-[8px] font-black text-black uppercase">
                            {t.amount}
                          </span>
                          <span className="text-lg font-black text-black font-mono">
                            {req.total_refund_amount.toFixed(2)}
                          </span>
                        </div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar bg-white/30">
          {cart.length > 0 ? (
            cart.map((item) => (
              <div
                key={(item as any).cartId}
                className="bg-white  p-3 rounded-2xl border border-orange-50  shadow-sm flex gap-3 group animate-in slide-in-from-bottom-2"
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                  ...
                  <div className="flex justify-between items-end">
                    <div className="flex flex-wrap gap-1">
                      {item.selectedCustomizations && (
                        <span className="px-1.5 py-0.5 bg-white  rounded text-[9px] font-bold text-black uppercase">
                          {item.selectedCustomizations.size}
                        </span>
                      )}
                      {item.selectedCustomizations?.selectedAddOns?.map((ao: AddOn) => (
                        <span
                          key={ao.id}
                          className="px-1.5 py-0.5 bg-white  rounded text-[9px] font-bold text-black uppercase"
                        >
                          +{ao.name}
                        </span>
                      ))}
          ...
          <div className="space-y-2 mb-6">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-dashed border-orange-100 ">
              ...
              <span className="text-3xl font-black font-mono">
                {totals.total.toFixed(2)}{' '}
                <span className="text-xs font-bold opacity-50">{t.currency}</span>
              </span>
            </div>
          </div>
              ...
              {lastTransaction.change_amount > 0 && (
                <div className="flex justify-between items-center border-t border-dashed border-orange-100  pt-3 text-black ">
                  ...
                  <span className="text-lg font-black">
                    {lastTransaction.change_amount.toFixed(2)} {t.currency}
                  </span>
                </div>
              )}
          ...
          <div className="bg-white  rounded-[48px] max-w-md w-full p-10 shadow-2xl animate-in zoom-in-95 border border-orange-100  relative">
            ...
            <div className="bg-white  p-6 rounded-[32px] mb-8 border border-orange-50 ">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-2xl font-black text-orange-600 font-mono">
                  {pendingReturnRequest.total_refund_amount.toFixed(2)} {t.currency}
                </span>
              </div>
            ...
            <div className="bg-white  p-6 rounded-[32px] mb-8 border border-orange-50  space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-mono font-bold">
                  {currentShift.initial_cash.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-3xl font-mono font-black">
                  {shiftTotals.expected.toFixed(2)}{' '}
        ...
        <div className="fixed inset-0 bg-white/80 backdrop-blur-xl z-[510] flex items-center justify-center p-4">
          <div className="bg-white  rounded-[40px] max-w-sm w-full p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-xs font-bold text-black uppercase mb-2 block">
                  {t.amount}
                </label>
                <input
                  type="number"
          ...
          <div className="bg-white  rounded-[48px] max-w-md w-full p-10 shadow-2xl animate-in zoom-in-95 border border-orange-100 ">
            {!shiftReport ? (
              ...
              <>
                <div className="text-center mb-6">
                  <div
                    className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border ${Math.abs(shiftReport.discrepancy) < 0.5 ? 'bg-white text-green-600 border-green-200' : 'bg-white text-red-600 border-red-200'}`}
                  >
                    {Math.abs(shiftReport.discrepancy) < 0.5 ? (
                ...
                <div className="bg-white  p-6 rounded-[32px] mb-8 space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`text-2xl font-mono font-black ${Math.abs(shiftReport.discrepancy) > 0.5 ? 'text-red-600' : 'text-green-600'}`}
                    >
                      {shiftReport.discrepancy > 0 ? '+' : ''}
                      {shiftReport.discrepancy.toFixed(2)}
                    </span>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-white/20">
            ...
            <div className="p-6">
              <div className="relative mb-6">
                ...
                {isSearchingCustomer && (
                  <Loader2
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-orange-600 animate-spin"
                    size={20}
                  />
                )}
              </div>
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (err.code === '23505') showErrorToast('Phone already exists.');
                            else showErrorToast('Failed to create an empty profile.');
                          } finally {
                            setIsSearchingCustomer(false);
                          }
                        }}
