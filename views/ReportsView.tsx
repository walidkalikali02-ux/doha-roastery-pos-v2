...
const ReportsView: React.FC = () => {
  ...
  const [cashierSales, setCashierSales] = useState<
    Array<{ cashier_name: string; sales_count: number; total_amount: number }>
  >([]);
  const [cashierSalesLoading, setCashierSalesLoading] = useState(false);
  const [paymentMethodData, setPaymentMethodData] = useState<
    Array<{ method: string; count: number; total: number }>
  >([]);
  const [cashierProductSales, setCashierProductSales] = useState<
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const anyCostVisible = (data || []).some(
          (row: any) => row.total_cost !== null && row.total_cost !== undefined
        );
        if (!anyCostVisible) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) showError(t.actionFailed || 'Failed to load cashier data');
      } finally {
    ...
    const load = async () => {
      ...
      try {
        ...
        const [
          ...
          supabase
            .from('monthly_production_report')
            .select('*')
            .order('month', { ascending: false })
            .limit(12),
          supabase
            .from('waste_by_roast_profile_report')
            .select('*')
            .gte('month', monthStart.toISOString())
            .lt('month', nextMonthStart.toISOString())
            .order('waste_percent', { ascending: false })
            .limit(10),
          supabase
            .from('waste_by_roaster_report')
            .select('*')
            .gte('month', monthStart.toISOString())
            .lt('month', nextMonthStart.toISOString())
            .order('waste_percent', { ascending: false })
          ...
          supabase
            ...
            .limit(10),
          supabase
            .from('qc_monthly_report')
            .select('*')
            .order('month', { ascending: false })
            .limit(12),
          supabase
            .from('roaster_performance_monthly_report')
            .select('*')
            .order('month', { ascending: false })
            .limit(50),
        ]);
    ...
    const load = async () => {
      ...
      try {
        ...
        const [costRes, consumptionRes, consistencyRes] = await Promise.allSettled([
          supabase
            .from('production_cost_report')
            .select('*')
            .gte('roast_date', since30Key)
            .order('roast_date', { ascending: false })
            .limit(50),
          supabase
            .from('green_bean_consumption_monthly_report')
            .select('*')
            .order('month', { ascending: false })
            .limit(120),
          supabase
            .from('roast_profile_consistency_report')
            .select('*')
            .order('month', { ascending: false })
            .limit(120),
        ]);
        ...
        const nextCost =
          costRes.status === 'fulfilled' && !costRes.value.error ? costRes.value.data || [] : [];
        const nextConsumption =
          consumptionRes.status === 'fulfilled' && !consumptionRes.value.error
            ? consumptionRes.value.data || []
            : [];
        const nextConsistency =
          consistencyRes.status === 'fulfilled' && !consistencyRes.value.error
            ? consistencyRes.value.data || []
            : [];
  ...
  const pieData = profitabilityRows
    .slice(0, 4)
    .map((row) => ({ name: row.name, value: Math.max(0, Math.round(row.revenue)) }));
  ...
  const roasterPerfThisMonth = latestMonthly?.month
    ? roasterPerformance.filter((r: any) =>
        String(r.month).startsWith(String(latestMonthly.month).slice(0, 7))
      )
    : roasterPerformance.slice(0, 10);
  const latestConsumptionMonth = greenBeanConsumption[0]?.month
    ? String(greenBeanConsumption[0].month).slice(0, 7)
    : null;
  ...
  const latestConsistencyMonth = recipeConsistency[0]?.month
    ? String(recipeConsistency[0].month).slice(0, 7)
    : null;
    ...
    sections.push({
      ...
      rows: dailyProduction
        ...
        .map((r: any) => [
          ...
          r.waste_percent !== null && r.waste_percent !== undefined
            ? `${Number(r.waste_percent).toFixed(2)}%`
            : '',
        ]),
    });
      ...
      sections.push({
        ...
        rows: productionCost
          ...
          .map((r: any) => [
            ...
            r.roaster_name,
            r.total_batch_cost !== null && r.total_batch_cost !== undefined
              ? Number(r.total_batch_cost).toFixed(2)
              : '',
            r.cost_per_packaged_unit !== null && r.cost_per_packaged_unit !== undefined
              ? Number(r.cost_per_packaged_unit).toFixed(4)
              : '',
          ]),
      });
    }
    ...
    if (consistencyThisMonth.length > 0) {
      sections.push({
        ...
        columns: [
          ...
          { label: t.roastProfile },
          { label: t.usageCount },
          { label: t.wasteCvPercent },
          { label: t.chargeCvPercent },
          { label: t.roastTimeWithin30s },
        ],
        rows: consistencyThisMonth
          .slice(0, 60)
          .map((r: any) => [
            String(r.month).slice(0, 7),
            r.roast_profile_name,
            r.usage_count,
            r.waste_cv_percent !== null && r.waste_cv_percent !== undefined
              ? Number(r.waste_cv_percent).toFixed(2)
              : '',
            r.charge_cv_percent !== null && r.charge_cv_percent !== undefined
              ? Number(r.charge_cv_percent).toFixed(2)
              : '',
            r.roast_time_within_30s_rate !== null && r.roast_time_within_30s_rate !== undefined
              ? `${Number(r.roast_time_within_30s_rate).toFixed(1)}%`
              : '',
          ]),
      });
    }
  ...
  if (user?.role === UserRole.CASHIER) {
    return (
      <div className="p-6">
        <PersonalStatsPanel userId={user.id} userName={user.name || ''} t={t} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in zoom-in-95 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-orange-900 transition-colors">
            {t.reports}
          </h2>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 w-full md:w-auto">
          <div className="flex-1 md:flex-none flex bg-white border border-orange-100 rounded-xl px-4 py-2 md:py-3 items-center gap-3 shadow-sm transition-colors">
            ...
            <span className="text-xs md:text-sm font-bold whitespace-nowrap text-orange-900">
              {t.last30Days}
            </span>
          </div>
          <button
            onClick={handleExportExcel}
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white border border-orange-100 text-orange-900 px-4 md:px-6 py-2 md:py-3 rounded-xl font-bold shadow-sm text-xs md:text-sm whitespace-nowrap active:scale-95 transition-all"
          >
            <FileDown size={18} />
            {t.exportExcel}
          </button>
          <button
            ...
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-orange-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-xl font-bold shadow-lg  text-xs md:text-sm whitespace-nowrap active:scale-95 transition-all"
          >
            <FileDown size={18} />
            {t.exportPdf}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="bg-white  p-5 md:p-8 rounded-2xl shadow-sm border border-stone-200  transition-colors">
          <h3 className="text-base md:text-lg font-bold mb-6 text-stone-800 ">
            {t.salesDistribution}
          </h3>
          <div className="h-64">
          ...
          <div className="space-y-5">
            {profitabilityLoading ? (
              ...
              profitabilityRows.map((product, idx) => (
                <div
                  key={idx}
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 group"
                >
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <span className="font-bold text-stone-700  text-xs md:text-sm line-clamp-1">
                      {product.name}
                    </span>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-4 md:gap-8 grow">
                    <div className={`text-${t.dir === 'rtl' ? 'right' : 'left'} shrink-0`}>
                      ...
                      <span
                        className={`text-xs md:text-sm font-bold ${product.trend === 'up' ? 'text-black ' : 'text-stone-500 '}`}
                      >
      ...
      <div className="bg-white  p-5 md:p-8 rounded-2xl shadow-sm border border-stone-200  transition-colors">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <h3 className="text-base md:text-lg font-bold text-stone-800 ">
              {t.cashManagementReport}
            </h3>
          ...
          <div className="bg-white p-4 md:p-6 rounded-xl border border-orange-100">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white border border-amber-200  rounded-lg p-3 mt-4">
                <p className="text-xs text-amber-700 ">{t.minorDiscrepancy}</p>
              </div>
            </div>
          </div>
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono font-bold">
                      {latestDaily ? new Date(latestDaily.day).toLocaleDateString() : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.batchCount}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestDaily ? Number(latestDaily.batch_count || 0) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.totalWeightKg}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestDaily ? Number(latestDaily.total_output_kg || 0).toFixed(2) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.wastePercent}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestDaily && latestDaily.waste_percent !== null
                        ? `${Number(latestDaily.waste_percent).toFixed(2)}%`
                        : '-'}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-white p-4 md:p-6 rounded-xl border border-stone-100">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono font-bold">
                      {latestMonthly ? String(latestMonthly.month).slice(0, 7) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.batchCount}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestMonthly ? Number(latestMonthly.batch_count || 0) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.totalWeightKg}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestMonthly ? Number(latestMonthly.total_output_kg || 0).toFixed(2) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.wastePercentDelta}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestMonthly && latestMonthly.waste_percent_delta !== null
                        ? `${Number(latestMonthly.waste_percent_delta).toFixed(2)}%`
                        : '-'}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-white p-4 md:p-6 rounded-xl border border-stone-100">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.month}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestQcMonthly ? String(latestQcMonthly.month).slice(0, 7) : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-stone-500">{t.passRate}</span>
                    <span className="text-sm font-mono font-bold">
                      {latestQcMonthly && latestQcMonthly.pass_rate_percent !== null
                        ? `${Number(latestQcMonthly.pass_rate_percent).toFixed(1)}%`
                        : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono font-bold">
                      {latestQcMonthly && latestQcMonthly.avg_score !== null
                        ? Number(latestQcMonthly.avg_score).toFixed(1)
                        : '-'}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {wasteByProfile.length === 0 ? (
                    ...
                    wasteByProfile.map((row: any) => (
                      <div
                        key={`${row.roast_profile_id || 'none'}-${row.month}`}
                        className="flex justify-between items-center"
                      >
                        <span className="text-xs font-bold text-stone-700 line-clamp-1">
                          {row.roast_profile_name || t.unknown}
                        </span>
                        <span className="text-xs font-mono font-bold">
                          {row.waste_percent !== null
                            ? `${Number(row.waste_percent).toFixed(2)}%`
                            : '-'}
                        </span>
                      </div>
                    ))
                ...
                <div className="space-y-2">
                  {wasteByRoaster.length === 0 ? (
                    ...
                    wasteByRoaster.map((row: any) => (
                      <div
                        key={`${row.roaster_id || 'none'}-${row.month}`}
                        className="flex justify-between items-center"
                      >
                        <span className="text-xs font-bold text-stone-700 line-clamp-1">
                          {row.roaster_name || t.unknown}
                        </span>
                        <span className="text-xs font-mono font-bold">
                          {row.waste_percent !== null
                            ? `${Number(row.waste_percent).toFixed(2)}%`
                ...
                <div className="space-y-2">
                  {wasteByBean.length === 0 ? (
                    ...
                    wasteByBean.map((row: any) => (
                      <div
                        key={`${row.bean_id || 'none'}-${row.month}`}
                        className="flex justify-between items-center"
                      >
                        <span className="text-xs font-bold text-stone-700 line-clamp-1">
                          {[row.origin, row.variety].filter(Boolean).join(' - ') || t.unknown}
                        </span>
                        <span className="text-xs font-mono font-bold">
                          {row.waste_percent !== null
                            ? `${Number(row.waste_percent).toFixed(2)}%`
            ...
            <div className="bg-white  p-4 md:p-6 rounded-xl border border-stone-100 ">
              ...
              {roasterPerfThisMonth.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table
                    className={`w-full ${t.dir === 'rtl' ? 'text-right' : 'text-left'} text-sm`}
                  >
                    ...
                    <tbody className="divide-y divide-stone-100">
                      {roasterPerfThisMonth.slice(0, 12).map((row: any) => (
                        <tr key={`${row.month}-${row.roaster_id || row.roaster_name}`}>
                          ...
                          <td className="py-3 font-mono font-bold">
                            {row.pass_rate_percent !== null
                              ? `${Number(row.pass_rate_percent).toFixed(1)}%`
                              : '-'}
                          </td>
                          <td className="py-3 font-mono font-bold">
                  ...
                  >
                    ...
                    <tbody className="divide-y divide-stone-100">
                      {productionCost.slice(0, 12).map((row: any) => (
                        <tr key={row.batch_id}>
                          <td className="py-3 font-mono font-bold">
                            {String(row.roast_date).slice(0, 10)}
                          </td>
                          <td className="py-3 font-mono font-bold">
                            {row.total_batch_cost !== null && row.total_batch_cost !== undefined
                              ? Number(row.total_batch_cost).toFixed(2)
                              : '-'}
                          </td>
                          <td className="py-3 font-mono font-bold">
                            {row.cost_per_packaged_unit !== null &&
                            row.cost_per_packaged_unit !== undefined
                              ? Number(row.cost_per_packaged_unit).toFixed(4)
                              : '-'}
                          </td>
                        </tr>
                      ))}
              ...
              {consumptionThisMonth.length === 0 ? (
                ...
                <div className="space-y-2">
                  {consumptionThisMonth
                    .slice()
                    .sort(
                      (a: any, b: any) => Number(b.quantity_kg || 0) - Number(a.quantity_kg || 0)
                    )
                    .slice(0, 12)
                    .map((row: any) => (
                      <div
                        key={`${row.bean_id}-${row.month}`}
                        className="flex justify-between items-center"
                      >
                        <span className="text-xs font-bold text-stone-700 line-clamp-1">
                          {[row.origin, row.variety].filter(Boolean).join(' - ') || t.unknown}
                        ...
                        <span className="text-xs font-mono font-bold">
                          {Number(row.quantity_kg || 0).toFixed(2)} {t.kg}
                        </span>
                      </div>
                    ))}
                </div>
              ...
              {consistencyThisMonth.length === 0 ? (
                ...
                <div className="space-y-2">
                  {consistencyThisMonth
                    ...
                    .sort(
                      (a: any, b: any) =>
                        Number(a.waste_cv_percent ?? 0) - Number(b.waste_cv_percent ?? 0)
                    )
                    .slice(0, 12)
                    .map((row: any) => (
                      <div
                        key={`${row.roast_profile_id}-${row.month}`}
                      ...
                      >
                        ...
                        <span className="text-xs font-mono font-bold whitespace-nowrap">
                          {row.waste_cv_percent !== null && row.waste_cv_percent !== undefined
                            ? `${Number(row.waste_cv_percent).toFixed(2)}%`
                            : '-'}
                        </span>
                      </div>
                    ))}
                ...
                >
                  <div>
                    ...
                    <span className="text-xs text-stone-500 ml-2">
                      ({pm.count} {t.transactions || 'transactions'})
                    </span>
                  </div>
                  <span className="font-mono font-bold text-green-600">
                    {pm.total.toFixed(2)} QAR
                  </span>
                </div>
              ))}
          ...
          {cashierSalesLoading ? (
            ...
            <div className="space-y-3">
              {cashierProductSales.slice(0, 10).map((product, idx) => (
                <div
                  key={idx}
                  className="flex justify-between items-center p-3 bg-stone-50 rounded-xl"
                >
