...
const RoastingView: React.FC<{ onDetailOpen?: (id: string | null) => void }> = ({
  onDetailOpen,
}) => {
  ...
  const [monthlyConsumption, setMonthlyConsumption] = useState<
    Array<{ month: string; variety: string; totalKg: number }>
  >([]);
  ...
  const [batchToCancel, setBatchToCancel] = useState<RoastingBatch | null>(null);

  const [newBatchData, setNewBatchData] = useState({
    beanId: '',
    level: RoastingLevel.MEDIUM,
    preWeight: '',
    notes: '',
  });
        ...
        setTemplates(
          tempData.map(
            (tm) =>
              ({
                ...tm,
                sizeLabel: tm.size_label,
                weightInKg: tm.weight_in_kg,
                unitCost: tm.unit_cost,
                shelf_life_days: tm.shelf_life_days,
                skuPrefix: tm.sku_prefix,
      ...
      if (locData) {
        ...
        if (locData.length > 0)
          setProductionDates((prev) => ({
            ...prev,
            targetLocationId: locData.find((l) => l.is_roastery)?.id || locData[0].id,
          }));
      }
      if (movementData) {
  ...
  const mapBatchFromDB = (item: any): RoastingBatch => ({
    ...
    roastTime: item.roast_time,
    level: item.level as RoastingLevel,
    preWeight: item.pre_weight,
    postWeight: item.post_weight || 0,
    wastePercentage: item.waste_percentage || 0,
  ...
  const updatePackagingLine = (tempId: string, field: keyof PackagingLine, value: string) => {
    setPackagingLines(
      packagingLines.map((l) => (l.tempId === tempId ? { ...l, [field]: value } : l))
    );
  };
  ...
  const handleCreateProduction = async (e: React.FormEvent) => {
    ...
    if (totalBatchPackagingWeightNeeded > remaining + 0.001) {
      showError(t.insufficientBatchWeight);
      return;
    }
    ...
    try {
      ...
      console.error(err);
      showError(t.productionSyncFailed);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleStartBatch = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      roast_date: now.toISOString().split('T')[0],
      roast_time: now.toTimeString().split(' ')[0],
      level: newBatchData.level,
      pre_weight: pre,
      status: BatchStatus.PREPARATION,
      operator: user?.name || 'Roaster',
      notes: newBatchData.notes,
      cost_per_kg: selectedBean.cost_per_kg,
      history: [
        {
          timestamp: now.toLocaleString(),
          action: 'CREATE',
  ...
  const handleFinishBatch = async (batchId: string) => {
    ...
    const historyEntry = {
      ...
      action: 'UPDATE',
      operator: user?.name || 'Roaster',
      details: t.batchOutputRecorded
        .replace('{weight}', post.toString())
        .replace('{waste}', waste.toFixed(2)),
    };
    try {
      await supabase
        .from('roasting_batches')
        .update({
          post_weight: post,
          waste_percentage: parseFloat(waste.toFixed(2)),
  ...
  const handleCancelBatch = (batch: RoastingBatch) => {
    if (isSaving) return;
    const isUnstarted = !batch.postWeight || batch.postWeight <= 0;
    if (!isUnstarted) {
      showError(t.batchAlreadyStarted);
      return;
    }
  ...
  const confirmCancelBatch = async () => {
    ...
    try {
      ...
      await supabase
        ...
        .eq('id', batch.beanId);
      await supabase.from('green_bean_movements').delete().eq('batch_reference', batch.id);
      const historyEntry = {
        timestamp: new Date().toLocaleString(),
        action: 'DELETE',
        operator: user?.name || 'System',
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="w-full sm:w-auto bg-orange-600 text-white px-6 py-3 rounded-xl shadow-lg  font-bold flex items-center justify-center gap-2 transition-all active:scale-95"
        >
      ...
      {activeBatches.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-bold flex items-center gap-2">
            <Clock className="text-black " /> {t.activeBatches}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {activeBatches.map((batch) => (
              <div
                key={batch.id}
                className="bg-white  rounded-[32px] p-6 shadow-sm border border-orange-50  transition-all hover:shadow-md"
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h4 className="font-bold text-lg leading-tight">
                      {beans.find((b) => b.id === batch.beanId)?.label}
                    </h4>
                  </div>
                  <span className="px-3 py-1 bg-orange-600 text-white rounded-full text-[10px] font-bold animate-pulse uppercase tracking-widest">
                    {t.inProgress}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-6 text-sm">
                  <div className="bg-white  p-3 rounded-2xl">
                    <span className="text-[10px] block text-black font-bold uppercase">
                      {t.preWeight}
                    </span>
                    <span className="font-bold font-mono">{batch.preWeight}kg</span>
                  </div>
                  <div className="bg-white  p-3 rounded-2xl">
                    <span className="text-[10px] block text-black font-bold uppercase">
                      {t.roastLevel}
                    </span>
                    <span className="font-bold">{batch.level}</span>
                  </div>
                </div>
                {finishingBatchId === batch.id ? (
                  <div className="space-y-4">
                    <input
                      ...
                      className="w-full bg-white  border-none rounded-xl px-4 py-3 font-mono font-bold outline-none focus:ring-2 focus:ring-orange-600"
                      placeholder="Output kg"
                    />
                    <button
                      onClick={() => handleFinishBatch(batch.id)}
                      className="w-full bg-orange-600 text-white py-3 rounded-xl font-bold flex justify-center gap-2 items-center  transition-colors"
                    >
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={() => setFinishingBatchId(batch.id)}
                      className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all hover:scale-[1.02] hover"
                    >
                    ...
                    {(!batch.postWeight || batch.postWeight <= 0) && (
                      <button
                        ...
                        className="w-full py-3 border border-orange-600 text-black rounded-2xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
                      >
        ...
        <div className="p-6 border-b border-orange-50  flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white">
          <div>
            <h3 className="font-bold flex items-center gap-2">
              <LayoutList size={18} className="text-black " /> {t.monthlyConsumptionTitle}
            </h3>
          ...
          <table className={`w-full ${t.dir === 'rtl' ? 'text-right' : 'text-left'} text-sm`}>
            ...
            <tbody className="divide-y divide-orange-50 ">
              {monthlyConsumption.length === 0 ? (
                <tr>
                  <td className="px-8 py-6 text-black" colSpan={3}>
                    {t.noConsumptionData}
                  </td>
                </tr>
              ) : (
                monthlyConsumption.map((row) => (
                  <tr key={`${row.month}-${row.variety}`}>
                    ...
                    <td className="px-8 py-5 font-mono font-bold text-black">
      ...
      <div className="bg-white  rounded-[40px] shadow-sm border border-orange-100  overflow-hidden">
        <div className="p-6 border-b border-orange-50  flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white">
          <div>
            <h3 className="font-bold flex items-center gap-2">
              <History size={18} className="text-black " /> {t.roastingLogAvailable}
            </h3>
          </div>
          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => setShowOnlyReady(!showOnlyReady)}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${showOnlyReady ? 'bg-orange-600 text-white shadow-lg' : 'bg-white  border border-orange-100  text-black '}`}
            >
            ...
            <input
              ...
              className="bg-white  border border-orange-100  rounded-xl px-4 py-2 text-xs outline-none focus:ring-1 focus:ring-orange-600"
            />
          </div>
        </div>
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-orange-50 ">
              {filteredHistory.map((batch) => {
                ...
                return (
                  <tr
                    key={batch.id}
                    className=" /40 cursor-pointer group transition-colors"
                    onClick={() => setShowDetailsModal(batch)}
                  >
                    <td className="px-8 py-5 font-mono font-bold text-black group-hover:text-black ">
                      {batch.id}
                    </td>
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${isReady ? 'bg-orange-600 text-white  ' : 'border border-orange-600 text-black  '}`}
                      >
                      ...
                      <div className="flex items-center gap-2">
                        {isReady && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleOpenProduction(batch);
                            }}
                            className="bg-orange-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-md  active:scale-95 transition-all flex items-center gap-2"
                          >
                            <Package2 size={14} /> {t.package}
                          </button>
                        )}
                        <div className="p-1.5 rounded-full bg-white  text-black group-hover:text-black  transition-colors">
                          <ArrowRight size={14} className={t.dir === 'rtl' ? 'rotate-180' : ''} />
                        </div>
                      </div>
                    </td>
                  </tr>
      ...
      {showProductionModal && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-white/60 backdrop-blur-md">
          <div
            className="bg-white  rounded-[40px] max-w-4xl w-full p-8 shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh] custom-scrollbar"
            dir={t.dir}
          >
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-2xl font-bold flex items-center gap-3">
                  <Package2 className="text-black " /> {t.batchPackagingAllocation}
                </h3>
              </div>
              <button
                onClick={() => setShowProductionModal(null)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              <div className="bg-white  p-5 rounded-3xl border border-orange-100 ">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs font-bold text-black ">
                    {(
                      getBatchWeightStats(showProductionModal).remaining -
                      totalBatchPackagingWeightNeeded
                    ).toFixed(2)}{' '}
                    kg
                  </span>
                </div>
              ...
              <div
                className={`p-5 rounded-3xl border transition-all ${totalBatchPackagingWeightNeeded > getBatchWeightStats(showProductionModal).remaining ? 'bg-white border-orange-600  ' : 'bg-white border-orange-100  '}`}
              >
                <span className="text-[10px] uppercase font-black text-black block mb-1">
                  {t.weightNeeded}
                </span>
                <span
                  className={`text-2xl font-black font-mono ${totalBatchPackagingWeightNeeded > getBatchWeightStats(showProductionModal).remaining ? 'text-black  underline decoration-2' : 'text-black '}`}
                >
            ...
            <form
              ...
              className={`space-y-8 ${!hasProducts ? 'opacity-30 pointer-events-none grayscale' : ''}`}
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-black uppercase flex items-center gap-1">
                    <CalendarIcon size={12} /> {t.productionDate}
                  </label>
                  <input
                    type="date"
                    required
                    value={productionDates.productionDate}
                    onChange={(e) =>
                      setProductionDates({ ...productionDates, productionDate: e.target.value })
                    }
                    className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-orange-600"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-black uppercase flex items-center gap-1">
                    <CalendarIcon size={12} /> {t.packagingDate}
                  </label>
                  <input
                    type="date"
                    required
                    value={productionDates.packagingDate}
                    onChange={(e) =>
                      setProductionDates({ ...productionDates, packagingDate: e.target.value })
                    }
                    className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-orange-600"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-black uppercase flex items-center gap-1">
                    <MapPin size={12} /> {t.destinationLocation}
                  </label>
                  <select
                    required
                    value={productionDates.targetLocationId}
                    onChange={(e) =>
                      setProductionDates({ ...productionDates, targetLocationId: e.target.value })
                    }
                    className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-orange-600"
                  >
                    {locations.map((loc) => (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {packagingLines.map((line, index) => {
                    ...
                    const lineWeight = template
                      ? parseInt(line.quantity || '0') * template.weightInKg
                      : 0;

                    return (
                      <div
                        key={line.tempId}
                        className={`flex flex-col md:flex-row gap-4 p-4 rounded-2xl border transition-all animate-in slide-in-from-top-2 duration-300 bg-white/50 border-orange-100 `}
                      >
                        <div className="flex-1 space-y-1">
                          <select
                            ...
                            onChange={(e) =>
                              updatePackagingLine(line.tempId, 'productId', e.target.value)
                            }
                            className={`w-full bg-white  border-none rounded-xl px-3 py-3 font-bold outline-none focus:ring-2 focus:ring-orange-600`}
                          >
                        ...
                        <div className="w-full md:w-28 space-y-1">
                          <input
                            ...
                            value={line.quantity}
                            onChange={(e) =>
                              updatePackagingLine(line.tempId, 'quantity', e.target.value)
                            }
                            className="w-full bg-white  border-none rounded-xl px-3 py-3 font-mono font-bold outline-none focus:ring-2 focus:ring-orange-600"
                            min="1"
                          />
                        </div>
                        <div className="w-full md:w-28 space-y-1">
                        ...
                        <button
                          type="button"
                          onClick={() => removePackagingLine(line.tempId)}
                          disabled={packagingLines.length === 1}
                          className="p-3 text-black hover:text-black  disabled:opacity-30 transition-colors"
                        >
                          <Trash2 size={20} />
                        </button>
                      </div>
              ...
              <div className="flex gap-4 pt-6">
                <button
                  type="button"
                  onClick={() => setShowProductionModal(null)}
                  className="flex-1 font-bold text-black  transition-colors"
                >
                  {t.cancel}
                </button>
                <button
                  ...
                  disabled={
                    isSaving ||
                    totalBatchPackagingWeightNeeded <= 0 ||
                    totalBatchPackagingWeightNeeded >
                      getBatchWeightStats(showProductionModal).remaining
                  }
                  className="flex-[2] py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2  active:scale-95 transition-all disabled:opacity-50"
                >
      ...
      {showDetailsModal && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-white/60 backdrop-blur-md animate-in fade-in duration-300 overflow-y-auto">
          <div
            className="bg-white  rounded-[40px] max-w-4xl w-full shadow-2xl overflow-hidden relative my-8"
            dir={t.dir}
          >
            <div className="p-8 border-b border-orange-50  flex justify-between items-center bg-white/50 /40">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h3 className="text-2xl font-bold">{t.batchDetails}</h3>
                  <span className="text-black font-mono text-sm">{showDetailsModal.id}</span>
                </div>
              </div>
              <button
            ...
            <div className="p-8 space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white  p-6 rounded-[32px] border border-orange-50 ">
                  <span className="text-[10px] font-black uppercase text-black block mb-2">
                    {t.preWeight}
                  </span>
                  <span className="text-2xl font-black font-mono">
                    {showDetailsModal.preWeight}kg
                  </span>
                </div>
                <div className="bg-white  p-6 rounded-[32px] border border-orange-50 ">
                  <span className="text-[10px] font-black uppercase text-black block mb-2">
                  ...
                  <span className="text-2xl font-black font-mono">
                    {showDetailsModal.postWeight}kg
                  </span>
                </div>
                <div className="bg-white  p-6 rounded-[32px] border border-orange-50 ">
                  <span className="text-[10px] font-black uppercase text-black  block mb-2">
                  ...
                  <span className="text-2xl font-black font-mono text-black ">
                    {showDetailsModal.wastePercentage}%
                  </span>
                </div>
              </div>
              <div className="space-y-4">
                <h4 className="text-sm font-bold flex items-center gap-2 text-black ">
                  <Layers size={18} /> {t.packagingUnits}
                </h4>
                {showDetailsModal.packagingUnits.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {showDetailsModal.packagingUnits.map((unit) => (
                      <div
                        key={unit.id}
                        className="bg-white  border border-orange-100  rounded-2xl p-5 group flex items-center gap-4"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between mb-1">
                            <h5 className="font-bold text-xs truncate">
                              {products.find((p) => p.id === unit.productId)?.name || unit.size}
                            </h5>
                            <span className="text-[10px] font-mono text-black">{unit.sku}</span>
                          </div>
                          <div className="flex gap-2 text-[10px] text-black">
                            <span className="font-bold">
                              {unit.quantity} {t.units}
                            </span>
                            <span>
                              • {t.expShort} {unit.expiryDate}
                            </span>
                          </div>
                        </div>
                        <button
                          onClick={() => setShowPrintLabelModal(unit)}
                          className="p-3 bg-white  text-black hover:text-black  rounded-xl transition-all"
                        >
                          <Printer size={20} />
                        </button>
                      </div>
                  ...
                  <div className="p-8 text-center text-black bg-white/40 rounded-3xl border-2 border-dashed border-orange-50 ">
                    {t.noPackagingUnits}
                  </div>
                )}
              </div>
      ...
      {showPrintLabelModal && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-white/60 backdrop-blur-md animate-in fade-in duration-300">
          <div
            className="bg-white  rounded-[40px] max-w-md w-full p-8 shadow-2xl relative"
            dir={t.dir}
          >
            <div className="flex justify-between items-center mb-8 border-b border-orange-50  pb-4">
              <h3 className="text-xl font-bold flex items-center gap-2">
                <Printer className="text-black " /> {t.printPreview}
              </h3>
              <button onClick={() => setShowPrintLabelModal(null)} className="text-black hover">
                <X size={24} />
              </button>
            </div>
            <div
              id="label-to-print"
              className="bg-white border-2 border-orange-200 rounded-lg p-6 flex flex-col items-center text-black shadow-inner mb-8"
            >
              <div className="w-full flex justify-between items-start mb-4">
                <div className="bg-orange-600 text-white p-1 rounded font-black text-[8px] tracking-tighter uppercase">
                  Doha Roastery
                </div>
              ...
              <div className="bg-white p-2 rounded-lg border border-orange-50 mb-4">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${showPrintLabelModal.sku}`}
                  alt="QR Code"
                  className="w-32 h-32"
                />
              </div>
              <div className="w-full text-center space-y-1">
                <h4 className="font-bold text-sm leading-tight">
                  {products.find((p) => p.id === showPrintLabelModal.productId)?.name ||
                    showPrintLabelModal.size}
                </h4>
              ...
              <div className="w-full grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-orange-50 text-[8px] font-bold uppercase text-black">
                <div>
                  <span className="block text-[6px] opacity-60">{t.productionDate}</span>
                  {showPrintLabelModal.productionDate}
                </div>
                <div className="text-right">
                  <span className="block text-[6px] opacity-60">{t.expiryDate}</span>
                  {showPrintLabelModal.expiryDate}
                </div>
              </div>
            </div>
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-3 font-bold text-black  transition-colors"
              >
                {t.cancel}
              </button>
              <button
                onClick={() => {
                  window.print();
                  setShowPrintLabelModal(null);
                }}
                className="flex-[2] py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2  active:scale-95 transition-all"
              >
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-white/60 backdrop-blur-md">
          <div className="bg-white  rounded-[40px] max-w-xl w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-bold flex items-center gap-3">
                <Flame className="text-black " /> {t.startNewBatch}
              </h3>
              <button onClick={() => setShowNewBatchModal(false)} className="text-black">
                <X size={28} />
              </button>
            </div>
            <form onSubmit={handleStartBatch} className="space-y-6" dir={t.dir}>
              <div className="space-y-1">
                ...
                <select
                  required
                  value={newBatchData.beanId}
                  onChange={(e) => setNewBatchData({ ...newBatchData, beanId: e.target.value })}
                  className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-orange-600"
                >
                  <option value="">-- {t.selectBean} --</option>
                  {beans.map((b) => (
                    <option key={b.id} value={b.id}>
                      {b.label} ({b.quantity}kg)
                    </option>
                  ))}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    value={newBatchData.level}
                    onChange={(e) =>
                      setNewBatchData({ ...newBatchData, level: e.target.value as RoastingLevel })
                    }
                    className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-bold"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    step="0.1"
                    required
                    value={newBatchData.preWeight}
                    onChange={(e) =>
                      setNewBatchData({ ...newBatchData, preWeight: e.target.value })
                    }
                    placeholder="0.0 kg"
                    className="w-full bg-white  border-none rounded-2xl px-4 py-4 font-mono font-bold"
                  />
                </div>
              </div>
              <div className="flex gap-4 pt-4">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-[2] py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all hover"
                >
      ...
      <ConfirmationModal
        ...
        title={t.confirmCancelBatch || 'Cancel Batch'}
        message={t.confirmCancelBatch || 'Are you sure you want to cancel this batch?'}
        variant="danger"
      />
    </div>
  );
